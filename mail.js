(() => {
 ...
 async function api(path,data){const token=sessionStorage.getItem('kazior_v15_token')||'';const r=await fetch(path,{method:data?'POST':'GET',headers:{Authorization:'Bearer '+token,...(data?{'Content-Type':'application/json'}:{})},body:data?JSON.stringify(data):undefined});const result=await r.json();if(!r.ok)throw new Error(result.error||'Ошибка подключения');return result}
 ...
 async function connect(provider){
  if(['google','microsoft'].includes(provider)){
   try{const nonce=Array.from(crypto.getRandomValues(new Uint8Array(24)),x=>x.toString(16).padStart(2,'0')).join('');sessionStorage.setItem('kazior_oauth_nonce',nonce);const r=await api('/api/oauth/begin',{provider,purpose:'mail',nonce});location.assign(r.url)}catch(e){notice(e.message)}return;
  }
