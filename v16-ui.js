(() => {
  ...
  function settingsPanel(){
    ...
    if(!$('#v16Integrations')){
      ...
      <form data-integration="whatsapp" class="v16-panel"><h4>WhatsApp · подключить номер инженера</h4><p>Для каждого номера нужны его собственные idInstance и apiTokenInstance.</p><label>Инженер<select name="id"><option value="superadmin">Тимченко Евгений Юрьевич</option><option value="amuratova">Муратова Алма Зульфухаровна</option><option value="elnur">Нұрқадыр Елнұр Серікұлы</option><option value="altair">Мелис Альтаир Сержанович</option><option value="kuanysh">Досов Қуаныш Рыспекұлы</option><option value="abarakbaeva">Баракбаева Айжан Айтулгановна</option></select></label><label>idInstance<input name="id_instance" inputmode="numeric" autocomplete="off"></label><label>apiTokenInstance<input name="api_token" type="password" autocomplete="new-password"></label><label>Адрес API<input name="api_url" type="url" placeholder="https://api.green-api.com"></label><label><input name="enabled" type="checkbox" checked> Включить канал</label><button class="btn primary">Сохранить WhatsApp</button><p role="status"></p></form>
